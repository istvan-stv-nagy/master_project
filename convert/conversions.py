...
class Converter:
    ...
    def velo_2_image(self, x, y, z, width=1242, height=375, v_fov=(-24.9, 2.0), h_fov=(-50, 50)):
        image = np.zeros((height, width))
        image[::] = np.nan
        ...
        min_z, max_z = -1.9, -1.65
        mask = (x_image >= 0) & (x_image < width) & (y_image >= 0) & (y_image < height) & (x > 0) & (z > min_z) & (z < max_z)
        x_image = x_image[mask]
        y_image = y_image[mask]
        x = x[mask]
        y = y[mask]
        ...
        image[y_image, x_image] = z
        coords = np.vstack((x_image, y_image))
        color = image[~np.isnan(image)].reshape((1, -1))[0]
        return image, coords, color
